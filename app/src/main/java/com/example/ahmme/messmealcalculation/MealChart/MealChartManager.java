...
import com.example.ahmme.messmealcalculation.DataBaseHelper;
 ...
 */
public class MealChartManager {
    private DataBaseHelper helper;
    private SQLiteDatabase database;
    MealInfo mealInfo =new MealInfo();
    private Context context;

    public MealChartManager(Context context) {
    ...
    public String[] getAllName(int length) {
        this.open();
        String [] nameList=new String[length];
        Cursor cursor = database.query(DataBaseHelper.TABLE_MESS_MEAL, new String[]{DataBaseHelper.MESS_MEMBER_NAME},
                null, null, null, null, null);
        cursor.moveToFirst();
        String name = cursor.getString(cursor.getColumnIndex(DataBaseHelper.MESS_MEMBER_NAME));
        int i=0;
        nameList[i]=name;
        Toast.makeText(mealInfo.getContext(), name, Toast.LENGTH_SHORT).show();
        i=i+1;
        this.close();
        return nameList;
    }
